...
def goals (id):
	...
	def two_zero_halftime_team(home_goals_sorted, away_goals_sorted):

		if len(home_goals_sorted) < 2 and len(away_goals_sorted) < 2:
			return None
		elif not len(home_goals_sorted):
			if away_goals_sorted[1] < 45:
				two_zero_halftime = 'away'
			else:
				return None
		elif not len(away_goals_sorted):
			if home_goals_sorted[1] < 45:
				two_zero_halftime = 'home'
			else:
				return None
		elif len(home_goals_sorted) == 1:
			if away_goals_sorted[1] < home_goals_sorted[0] and away_goals_sorted[1] < 45:
				two_zero_halftime = 'away'
			else:
				return None
		elif len(away_goals_sorted) == 1:
			if home_goals_sorted[1] < away_goals_sorted[0] and home_goals_sorted[1] < 45:
				two_zero_halftime = 'home'
			else:
				return None
		else:
			if away_goals_sorted[1] < home_goals_sorted[0] and away_goals_sorted[1] < 45:
				two_zero_halftime = 'away'
			elif home_goals_sorted[1] < away_goals_sorted[0] and home_goals_sorted[1] < 45:
				two_zero_halftime = 'home'
			else:
				return None

		return two_zero_halftime
	...
	two_zero_halftime = two_zero_halftime_team(home_goals_sorted, away_goals_sorted)
...
def get_two_zero_halftime(games_id):
...
list_of_games_data = get_two_zero_halftime(games_id)

row_names = ['home_score', 'away_score', 'two_zero_halftime', 'home_team', 'away_team']

with open('two_zero_halftime.csv', 'w') as csv_file:
